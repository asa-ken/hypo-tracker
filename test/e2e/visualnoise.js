...
//
// 2026-08-22(2): 取り込み(貼り付け前・後)が18→14 / 41→37 に減った。①「このアプリの
// 説明書をAIに渡す」を既定で畳んだ状態にしたため(ユーザー指摘、使用頻度が低い)。
// display:noneの要素はCOUNT()の対象外なので、畳んだ分の要素が線の数から抜けた
//
// 2026-09-26: 銘柄詳細が208→204本に減った。上部の「注目 N」「メモ N」の件数バッジ(枠線つきのチップ)を
// 廃止したため(ユーザー指示「この数を強調することに大きな意味はない」)。市場・銘柄分析の行の
// 「メモ・注目 N」も廃止したが、そちらのチップは枠線を持たないため本数は11本のまま
const BASELINE = {
  'ホーム': 2,
  '市場・銘柄分析': 11,
  'リマインダー': 11,
