...
//
// 2026-08-22(2): 取り込み(貼り付け後)の高さが2014→1817px、colored が10.05→8.41に減った。
// ①「このアプリの説明書をAIに渡す」を既定で畳んだ状態にしたため(ユーザー指摘、使用頻度が低い)。
// ink は56.69→59.76に増えた(畳んだ分の白い塗りが減り、残った要素の地色の比率が相対的に上がったため)
//
// 2026-08-22(3): 取り込み(貼り付け後)の高さが1817→1872px(+55px)に増えた。Hit Area=1の修正
// (.candbox .tx/.swtapにmin-height:44pxを追加)により、1行だけの短い候補行が44pxまで
// 底上げされたため(実測、DECISIONS.md参照)。ink/coloredの微小な変化も同じ原因
//
// 2026-09-26: 件数バッジを廃止した(ユーザー指示「この数を強調することに大きな意味はない」)。
// 市場・銘柄分析は行の「メモ・注目 N」(navyのチップ)が消え、ink 17.80→16.62・colored 4.46→4.26。
// 銘柄詳細は上部の「注目 N」「メモ N」の行が消え、高さ2349→2311px(-38px)・ink 52.10→51.45・colored 8.38→8.25
const BASELINE = {
  'ホーム':                 { 高さ:  775, ink: 11.82, colored:  2.02 },
  '市場・銘柄分析':         { 高さ:  775, ink: 16.62, colored:  4.26 },
  'リマインダー':           { 高さ:  775, ink: 49.49, colored:  5.24 },
  '銘柄詳細':               { 高さ: 2311, ink: 51.45, colored:  8.25 },
  '取り込み(貼り付け後)':   { 高さ: 1872, ink: 60.70, colored:  8.19 },
};
