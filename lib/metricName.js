...
// 英語略称と日本語表記は文字が重ならないため、2文字組の一致率では拾えない
// (EPS と 1株当たり当期純利益、ROE と 自己資本利益率など)。既知の組だけ表で持つ
// (ユーザー報告、2026-08-13。実際にEPS/1株当たり当期純利益が別々に登録されていた)
const METRIC_ALIASES = [
  ['EPS', '1株当たり当期純利益', '1株当たり純利益'],
  ['ROE', '自己資本利益率'],
  ['PER(予想)', '予想株価収益率'],
  ['PBR', '株価純資産倍率'],
  ['FCF', 'フリーキャッシュフロー'],
];
function metAliasGroup(name) {
  const n = metNorm(name);
  return METRIC_ALIASES.find(g => g.some(x => metNorm(x) === n)) || null;
}
...
function metSim(a, b) {
  ...
  const ga = metAliasGroup(a);
  if (ga && ga.some(n => metNorm(n) === y)) return 1;  // 既知の英語略称⇔日本語表記
