...
function seed(){
  return {
    ...
    // 市場・テーマ用の指標。個別企業の指標(PER・売上高など)とは別物なので独立して持つ。
    // 現在値・当日の値動きではなく、数か月単位で見直す水準感・センチメントを対象にする
    marketMetricsMaster:{
...
function migrate(db){
  ...
  // 市場・テーマ用の指標マスター(旧データには存在しない)
  if(!db.marketMetricsMaster) db.marketMetricsMaster=s0.marketMetricsMaster;
  if(!db.marketMetricsMaster.snap) db.marketMetricsMaster.snap=[];
  if(!db.marketMetricsMaster.hiddenSnap) db.marketMetricsMaster.hiddenSnap=[];
