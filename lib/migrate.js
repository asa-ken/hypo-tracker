...
function seed(){
  return {
    ...
    marketMetricsMaster:{   // scope:'市場' … 日本株市場、米国株市場など
      snap:['予想PER(市場全体)','PBR(市場全体)','配当利回り(市場全体)','騰落レシオ(25日)','信用評価損益率','長期金利(10年)'],
      hiddenSnap:[]
    },
...
function migrate(db){
  ...
  // 業界の既定指標を財務指標に入れ替えた。ユーザーが編集していない(旧既定のままの)場合だけ差し替える
  {
    const OLD=['市場規模','市場成長率(YoY)','設備投資額'];
    const cur=db.industryMetricsMaster.snap||[];
    if(cur.length===OLD.length && cur.every((x,i)=>x===OLD[i])) db.industryMetricsMaster.snap=s0.industryMetricsMaster.snap.slice();
  }
