...
function seed(){
  return {
    ...
    marketMetricsMaster:{   // scope:'市場' … 日本株市場、米国株市場など
      ...
      snap:['予想PER(市場全体)','PBR(市場全体)','長期金利(10年)','為替(USD/JPY)','騰落レシオ(25日)','VIX指数'],
      hiddenSnap:[]
    },
    industryMetricsMaster:{ // scope:'業界' … 半導体、データセンターなど。個社ではなく業界の代表値(中央値)で見る
      snap:['PER','PBR','EBITDA','ROE','営業利益率','自己資本比率'],
      hiddenSnap:[]
    },
...
function migrate(db){
  ...
  // 既定指標を入れ替えた分は、ユーザーが編集していない(旧既定のままの)場合だけ差し替える
  {
    const RESET=[
      {k:'industryMetricsMaster', old:['市場規模','市場成長率(YoY)','設備投資額']},
      {k:'marketMetricsMaster', old:['予想PER(市場全体)','PBR(市場全体)','配当利回り(市場全体)','騰落レシオ(25日)','信用評価損益率','長期金利(10年)']},
    ];
    RESET.forEach(({k,old})=>{
      const cur=db[k].snap||[];
      if(cur.length===old.length && cur.every((x,i)=>x===old[i])) db[k].snap=s0[k].snap.slice();
    });
  }
