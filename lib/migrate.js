...
function migrate(db){
  ...
  (db.stocks||[]).forEach(st=>{
    if(st.mine&&st.mine.length){
      ...
      st.mine.forEach(m=>{ db.hypotheses.push({id:'h'+Math.random().toString(36).slice(2,9), stockId:st.id, kind:'memo', text:m.t, eventDate:null, eventNote:null, eventType:'none', createdAt:m.ts}); });
      delete st.mine;
    }
  });
  ...
  db.hypotheses=(db.hypotheses||[]).map(h=>{
    ...
    return {
      ...
      eventType:h.eventDate?'date':'none', createdAt:h.createdAt
    };
  });
  // 誤登録カードのクリーニング
