...
function seed(){
  return {
    ...
    // 業績推移側だけに置く(ユーザー指摘・確認の上、2026-08-13)。グラフは既定でOFFにする
    // (graph:trueにすると上限4件のうち最後の1枠を既定で埋めてしまい、既存の「3件が既定」という
    // 前提を崩すため。表(グラフ無し)には出るので、必要ならユーザーがグラフに切り替えられる)
    metricsMaster:{
      snap:['PER(予想)','PBR','自己資本比率','配当利回り(予想)'],
      trend:[{name:'売上高',graph:true},{name:'営業利益',graph:true},{name:'EPS',graph:true},{name:'ROE',graph:false}],
...
function migrate(db){
  ...
  // 業績推移名(売上高)を正として統合する
  {
    ...
    // GRAPH_MAXはこの時点で未定義の可能性があるため値(4)をそのまま使う。
    // ROEは既定でグラフ非表示(seedと同じ理由: 上限4件の最後の1枠を既定で埋めないため)
    if(!mm.trend.some(t=>t.name==='EPS')) mm.trend.push({name:'EPS', graph:mm.trend.filter(t=>t.graph).length<4});
    if(!mm.trend.some(t=>t.name==='ROE')) mm.trend.push({name:'ROE', graph:false});
