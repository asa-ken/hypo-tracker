...
function migrate(db){
  ...
  // 一覧には残したまま名前で印を付けるだけに改め、旧データも一覧へ戻す
  {
    ...
    ht.forEach(x=>{
      ...
      if(!mm.trend.some(t=>t.name===name)) mm.trend.push({name, graph:!!(x&&x.graph)});
      if(!mm.hiddenTrend.includes(name)) mm.hiddenTrend.push(name);
    });
    [mm, db.marketMetricsMaster, db.industryMetricsMaster, db.themeMetricsMaster].forEach(m=>{
      if(!m) return;
      m.snap=m.snap||[];
      m.hiddenSnap=(m.hiddenSnap||[]).map(x=>(typeof x==='string')?x:(x&&x.name)).filter(Boolean);
      m.hiddenSnap.forEach(k=>{ if(!m.snap.includes(k)) m.snap.push(k); });
    });
  }
