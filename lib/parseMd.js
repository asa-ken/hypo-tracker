...
function parseMd(text, ctx){
  ...
  for(let raw of lines){
    ...
    if(m=line.match(/^#\s*テーマ[:：]\s*(.+)$/)){ const nm=m[1].trim(); const mk=findMarketStockByName(nm); stk={name:nm, code:mk?mk.id:nm, isMarket:true}; flushFree(); continue; }
  ...
  rows.forEach(r=>{
    if(r.block!=='注目ポイント'){ if(pending){merged.push(pending);pending=null;} merged.push(r); return; }
    if(r.key==='本文'){ if(pending) merged.push(pending); pending={block:'注目ポイント', key:'注目ポイント', value:r.value, eventDate:null, raw:r.raw}; }
    else if(/イベント日/.test(r.key) && pending){ pending.eventDate=r.value; }
    else { if(pending){merged.push(pending);pending=null;} merged.push(r); }
  });
